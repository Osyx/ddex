...
describe("parseExport", () => {
  test("reads messages from lowercase 'messages' directory", async () => {
  ...
  test("reads messages from uppercase 'Messages' directory", async () => {
  ...
  test("reads a single JSON file directly", async () => {
    const file = join(FIXTURES, "export/messages/c100000000000000001/messages.json");
    const contents = await collect(file);
    expect(contents.length).toBe(8);
  });

  test("reads a second single JSON file directly", async () => {
